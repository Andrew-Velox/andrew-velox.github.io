...
export default function ChatBot() {
  ...
  const hideOnRoute = !KNOWN_ROUTES.has(pathname);
  ...
  // Keep pathname in sync across View Transitions swaps.
  useEffect(() => {
  ...
  const sendMessage = async () => {
    ...
    try {
      const requestBody: { question: string; session_id?: string | null } = { question: questionText };
      ...
      const botMessage: Message = {
        ...
        text: data.status === 'success' && data.answer ? data.answer : 'Sorry, I encountered an error. Please try again.',
        sender: 'bot',
        timestamp: new Date(),
      };
    ...
    <>
      {hideOnRoute ? null : (
        <>
            ...
            <div className="bg-white/[0.03] backdrop-blur-3xl saturate-150 rounded-2xl shadow-2xl shadow-black/60 border border-white/15 ring-1 ring-inset ring-white/10 w-80 md:w-96 h-[450px] md:h-[500px] flex flex-col overflow-hidden">
              <div className="bg-white/[0.04] backdrop-blur-2xl border-b border-white/10 p-3 md:p-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={clearConversation} className="text-white/80 hover:text-white transition-colors" title="Clear conversation">
              ...
              <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4 bg-white/[0.02] backdrop-blur-2xl">
                {messages.map((message) => (
                  <div key={message.id} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] rounded-2xl px-3 py-2 md:px-4 ${
                      message.sender === 'user'
          ...
          >
            <svg className="w-5 h-5 md:w-6 md:h-6 text-white transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
            </svg>
