...
const achievementsData: Section[] = [
  {
...
function AchievementCard({ item }: { item: Item }) {
  return (
    <FadeIn direction="up" duration={500} className="relative flex items-start">
      ...
      <div className="ml-16 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-6 flex-1 transition-transform duration-300 hover:scale-[1.02] hover:shadow-xl bg-white/10 dark:bg-white/5 backdrop-blur-md">
        ...
        {item.rank && (
          <div className="italic text-green-400 font-bold text-sm mb-1">Rank: {item.rank}</div>
        )}
      </div>
    </FadeIn>
  );
          ...
          <div className="space-y-16">
            {achievementsData.map((section) => (
              <div key={section.section}>
                <h2 className="text-2xl font-bold mb-6 text-white">{section.section}</h2>
