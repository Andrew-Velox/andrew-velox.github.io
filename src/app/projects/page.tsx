          ...
          {projects.map((project, i) => (
            <FadeIn key={project.id} delay={i * 80}>
              ...
              >
                ...
                <span className="absolute -top-2 -left-2 w-4 h-4 border-t-2 border-l-2 border-white/15 group-hover:border-red-600 transition-colors duration-300 z-10" />
                <span className="absolute -bottom-2 -right-2 w-4 h-4 border-b-2 border-r-2 border-white/15 group-hover:border-red-600 transition-colors duration-300 z-10" />

                <div className="border border-white/10 bg-white/[0.02] overflow-hidden transition-all duration-500 hover:border-white/25">
                  {/* Project Image */}
                  <div className="relative h-56 bg-[#111] overflow-hidden">
                    {project.image ? (
                      <Image
                        ...
                        alt={project.title}
                        fill
                        className="object-cover grayscale-[35%] contrast-125 transition-all duration-700 group-hover:grayscale-0 group-hover:scale-105"
                      />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center">
                        <span className="text-7xl font-bold text-white/5 select-none">
                          {kanjiNumerals[i] ?? project.id}
                        </span>
                      </div>
                    )}
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex flex-wrap gap-2 mb-8">
                      {project.tags.map((tag, index) => (
                        <span
                          key={index}
                          className="text-[0.7rem] tracking-wide px-2.5 py-1 border border-white/10 text-gray-300 font-mono uppercase"
                        >
                          {tag}
                        </span>
                      ))}
